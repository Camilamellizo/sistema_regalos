          ...
          <div class="flex flex-wrap w-1/4">
            <div class="w-full p-1 md:p-2 " v-for="(project, index) in projects" :key="index">
              <img alt="gallery" @click="guardarImagen(index)"
                class="block object-cover object-center w-full h-full rounded-lg border border-white hover:shadow-inner focus:outline-none transition duration-500 ease-in-out  transform hover:-translate-x hover:scale-105"
                :src=project.image>
            </div>
          </div>
        </div>
...
export default {
  ...
  methods: {
    ...
    guardarImagen(index) {
      ...
      if (this.addImage.length < 4) {
        const newImage = this.$allProjects[index]
        this.addImage.push(newImage)
      } else {
